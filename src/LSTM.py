...
DATA_PATH = r"\scaler_features.csv"
...
for outer_fold, (train_idx, test_idx) in enumerate(outer_cv.split(X_seq, y_seq, groups=ids)):
    ...
    print(
        f"Inner CV - MAE: {np.mean(inner_mae_list):.3f}±{np.std(inner_mae_list):.3f},RMSE: {np.mean(inner_rmse_list):.3f}±{np.std(inner_rmse_list):.3f}, R2: {np.mean(inner_r2_list):.3f}±{np.std(inner_r2_list):.3f}")
