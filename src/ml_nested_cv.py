...
DATA_FILE = BASE_PATH / "scaler_features.csv"
...
def main():
    ...
    best_hyperparams_for_deployment = {name: None for name in models}
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(features, target, groups=groups)):
        ...
        for model_name, (model_class, hyper_space) in models.items():
            ...
            if rmse < best_models_info[model_name]['rmse']:
                ...
                best_hyperparams_for_deployment[model_name] = best_params
    ...
    print("=== Training Final Models for Deployment on Full Dataset ===")

    for model_name, (model_class, _) in models.items():
        print(f"\nTraining final {model_name.upper()} model on the entire dataset...")

        deployment_params = best_hyperparams_for_deployment.get(model_name)

        if deployment_params is None:
            print(
                f"Warning: No best hyperparameters found for {model_name}. Skipping final model training for this type.")
            continue

        # Adjust hyperparameters one last time with the full feature set
        final_params = adjust_hyperparameters(model_class, deployment_params, features.shape[1])
        final_model = model_class(**final_params)

        final_model.fit(features, target)

        final_model_path = MODEL_DIR / f"{model_name}.pkl"
        joblib.dump(final_model, final_model_path)
        print(f"Final {model_name.upper()} model saved to {final_model_path}")
        print(f"Used hyperparameters: {final_params}")
